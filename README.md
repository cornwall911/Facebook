...
* **توليد كابشن تسويقي قصير بالذكاء الاصطناعي (AI Short Post Copywriter):** نظام مخصص لكتابة بوستات تسويقية قصيرة ومركزة (35-65 كلمة) للمنتجات الرابحة بأسلوب طبيعي ومجرب (3 Layers: Brand Brain + Viral Hooks + Quality Gate) عبر Google Gemini API.
* **مخرجات فورية متعددة:**
  1. **ملف Excel (`viral_posts.xlsx`):** يحتوي على الروابط والتفاعلات والكابشن الأصلي والكابشن التسويقي المولد.
  2. **قاعدة بيانات JSON (`viral_posts.json`):** داتا مهيكلة للأتمتة مع الكابشن الجاهز للنشر.
  3. **لوحة تحكم تفاعلية (`dashboard.html`):** تفتح في المتصفح لاستعراض البوستات، نسخ كابشن النشر بنقرة واحدة (1-Click Copy)، ومطابقة الصور في Google Lens.
...
│   ├── post_copywriter.py    # توليد الكابشن التسويقي القصير (Gemini API)
...
* لتوليد/تحديث كابشن البوستات الذكي للمنشورات المحفوظة مسبقاً:
```bash
python -m fb_winner_scout generate-captions
```
